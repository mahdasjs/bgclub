...
class HomePageEventListAPIView(generics.ListAPIView):
    serializer_class = ListEventSerializer
    
    def get_queryset(self):
        queryset = []
        for user in self.request.user.user_profile.follow.all():
            queryset.extend(list(Event.objects.filter(user=user)))
        queryset.sort(key=lambda r: r.date, reverse=True)
        return queryset

class HomePageEventRetrieveAPIView(generics.RetrieveAPIView):
...
class EventRetrieveUpdateDestroyAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        instance.event_len = request.data.get('event_len', instance.event_len)
        instance.event_lon = request.data.get('event_lon', instance.event_lon)
        instance.save()
